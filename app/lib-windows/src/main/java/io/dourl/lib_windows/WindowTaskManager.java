...
public class WindowTaskManager {
    ...
    private static final String TAG = "WindowTaskManager" ;
     ...
     * @param windowWrapper
     */
    public synchronized void showWindow(AppCompatActivity activity, FragmentManager manager, WindowWrapper windowWrapper) {
     ...
     */
    public synchronized void continueShow(AppCompatActivity activity) {
        if (!enable) {
            return;
        }
        ...
        if (windowWrapper != null && !windowWrapper.isWindowShow() && windowWrapper.isCanShow()) {
            ...
            if (window != null && isActivityAlive(activity)) {
                ...
                windowWrapper.getWindow().setOnWindowDismissListener(() -> {
                    if (windowWrapper.getWindow().getWindowName().equalsIgnoreCase(windowWrapper.getWindowName())) {
                        windowWrapper.setWindowShow(false);
                        mWindows.remove(windowWrapper.getWindowName());
                        Log.d(TAG,"remove{continueShow}"+windowWrapper.getWindowName());
                        if (windowWrapper.isAutoShowNext()) {
                            continueShow(activity);
                        }
                    }
                });
     ...
     * @param windowWrapper
     */
    private synchronized void show(AppCompatActivity activity, FragmentManager manager, WindowWrapper windowWrapper) {
        if (!enable) {
            return;
        }
    ...
    private boolean isActivityAlive(AppCompatActivity activity) {
        if (activity == null
